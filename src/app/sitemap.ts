...
import { getKraji, getProgramskaPodrocja, getNovice } from '@/lib/queries'
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticki: MetadataRoute.Sitemap = [
    ...
    { url: `${base}/kandidat`, changeFrequency: 'monthly', priority: 0.8 },
  ...
  try {
    const [kraji, podrocja, novice] = await Promise.all([
      getKraji(),
